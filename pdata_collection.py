...
def extract_long_form_answer(pred, use_prompt=True):
    ...
    if use_prompt:
        ...
        if long_form_start == -1:
            long_form_start = pred.find("Long Form Answer")
        if long_form_start == -1:
            long_form_start = pred.find("Long-FormAnswer")
        if long_form_start == -1:
            long_form_start = pred.find("Conclusion")
        if long_form_start == -1:
            long_form_start = pred.find("conclusion")
...
def main():
    ...
    if "meditron" in args.model_name_or_path.lower() or "llama3" in args.model_name_or_path.lower() or "mistral" in args.model_name_or_path.lower():
        print("000")
        ...
        print("******Qwen2.5******")
        model_name_Q = "Qwen/Qwen2.5-0.5B"
    ...
    elif  model_name == "MedLLaMA_13B":
        print("***chaoyi-wu/MedLLaMA_13B***")
        model_name = "MedLLaMA_13B"
...
### 8.Long-Form Answer:
- {{Combine the above reasoning to accurately and comprehensively answer the question. }} 
- {{Provide a "long-form answer" that contains 400-500 words. The word count must not be less than 400 words.}} 
### END
...
"""
    ...
    for inst_idx ,inst in enumerate(train_examples):
        ...
        if args.use_prompt:
            query += "Please ensure that the output contains Long-Form Answer, Structured Chain of Thought:"
...
Structured content."""
        ...
        if "meditron" in args.model_name_or_path.lower() or "Llama-2" in args.model_name_or_path.lower() or "mistral" in args.model_name_or_path.lower() and "instruct" not in args.model_name_or_path.lower():
        ...
        elif "Llama-3.2-3B-Instruct" == model_name:
            ...
            outputs = model.generate(input_ids, max_new_tokens=4096, eos_token_id=terminators, do_sample=False, temperature=0.0, top_p=0.9)
            ...
            try:
                ...
                print(e)
        elif model_name_Q == "Qwen/Qwen2.5-0.5B":

            print("Qwen2.5 ",model_name_Q , query)
