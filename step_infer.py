...
def extract_long_form_answer(pred, use_prompt=True, step_infer_flag= False):
    """
    提取预测中的长篇回答。
    ...
    if use_prompt or step_infer_flag:
...
def run_inference(model, tokenizer, system, query, model_name):
    print("2", model_name)
...
def run_inference_llama3_1B(model, pipe, system, query, model_name):
    print("2", model_name)
...
def qwen_generate_response(query, qwen_system, tokenizer, model, max_new_tokens=512):
    """
    Generate a response using a given model and tokenizer based on a system prompt and user query,
    and measure the total inference time.

    Args:
        query (str): The user's input query.
        qwen_system (str): The system prompt or instructions for the model.
        tokenizer: The tokenizer used for text tokenization.
        model: The model used for generating responses.
        max_new_tokens (int): Maximum number of new tokens to generate.

    Returns:
        tuple: A tuple containing the generated response as a string and the total inference time in seconds.
    """
    print("Qwen2.5", query)
    ...
    # Create the text for tokenization
    ...
    # Extract the generated tokens by excluding the input tokens
    generated_ids = [
        output_ids[len(input_ids):] for input_ids, output_ids in zip(model_inputs.input_ids, generated_ids)
    ]

    # Decode the generated IDs into a string response
...
def main():
    ...
    parser.add_argument('--step_infer', action='store_true', help='Enable or disable step_infer, default is False')
    ...
    if "meditron" in args.model_name_or_path.lower() or "llama" in args.model_name_or_path.lower() or "mistral" in args.model_name_or_path.lower():
        ...
        print("******Qwen2.5******")
        model_name_Q = "Qwen/Qwen2.5-1.5B-Instruct"
...
##step_infer
##step1
    step_prompt1="""Task: You are a helpful assistant.Please provide your understanding of the problem.
...
Question: What is the relationship between Noonan syndrome and polycystic renal disease?
Understand the Question:
Noonan syndrome is a genetic disorder marked by unique facial features, short stature, heart defects, and developmental delays, affecting various body systems. Polycystic renal disease, especially autosomal dominant polycystic kidney disease (ADPKD), causes multiple kidney cysts, leading to enlargement and dysfunction. This inquiry investigates the potential connections between these two conditions, focusing on shared genetic or pathological mechanisms.
question:
"""
...
##step2
    step_prompt2="""Task: You are a helpful assistant.Please provide your understanding of the problem.
...
##step3
    step_prompt3="""
Task: You are a helpful assistant.Please analyze medical information. Please analyze medical information.
...
# ##step8
#     step_prompt8="""Task: You are a helpful assistant. 
# - Combine the above question and information and reasoning to accurately and comprehensively answer the question. 
# - Provide a "long-form answer" that contains 300-400 words. 
...
##step8
    step_prompt8="""Task: You are a professional doctor. 
- Please ensure that the output contains Long-Form Answer
- Please provide a comprehensive and accurate medical answer based on the given information and questions. 
- Provide a "long-form answer" that more than 300 words. 
- Please ignore information unrelated to the problem.

Question: What is the relationship between Noonan syndrome and polycystic renal disease?
long form answer:Noonan's syndrome describes a range of congenital anomalies affecting both sexes, previously categorized under different syndromes. Unlike Turner's syndrome, there’s no karyotype abnormality, though familial patterns are common. Key features include neck webbing, hypertelorism, a shield-shaped chest, and short stature, along with congenital heart issues like pulmonary stenosis and sexual infantilism, especially cryptorchidism in males. Rarely, renal anomalies may occur, such as duplications and hydronephrosis. We report an infant with many Noonan's syndromes signs who also exhibited renal failure due to renal dysplasia with cystic disease.
"""
    ...
    name_step_infer = ""
    if args.step_infer:
        name_step_infer = "step_infer"

    ##load dataset
    filename = f"./alignment-handbook/predictions/pdata_{model_name}_{eval_name}_sampling_{use_prompt}_{few_shot}_{name_step_infer}.jsonl_tmp"
    write_name = f"./alignment-handbook/predictions/pdata_{model_name}_{eval_name}_sampling_{use_prompt}_{few_shot}_{name_step_infer}.jsonl_tmp"
    ...
    for inst_idx ,inst in enumerate(train_examples):
        ...
        if args.step_infer:
            qwen_system = """You are an experienced doctor, please output including (1 Understand the Question:
2. Recall Relevant Medical Knowledge
3. Analyze Medical Information
4. Assess Impacts and Considerations
5. Provide Additional Relevant Information
...
Structured content."""
            ...
            query1= step_prompt1 + question + " Understand the Question(please less than 50 words): "
            query2= step_prompt2 + question + " Recall Relevant Medical Knowledge(please less than 50 words): "
            query3= step_prompt3 + question + " Analyze Medical Information(please less than 50 words): "
            query4= step_prompt4 + question + " Assess Impacts and Considerations(please less than 50 words): "
            query5= step_prompt5 + question + " Provide Additional Relevant Information(please less than 50 words): "
            query6= step_prompt6 + question + " Suggest Follow-Up Steps or Actions(please less than 50 words): "
            query7= step_prompt7 + question + " Reference Reliable Sources(please less than 50 words): "
            query_list = [query1,query2,query3,query4,query5,query6,query7]
            step_list = [" Understand the Question: ", " Recall Relevant Medical Knowledge: ", " Analyze Medical Information: ", " Assess Impacts and Considerations: ", " Provide Additional Relevant Information: ", " Suggest Follow-Up Steps or Actions: ","  Reference Reliable Sources: "]
        ...
        # if "meditron" in args.model_name_or_path.lower() or "Llama-2" in model_name or "mistral" in args.model_name_or_path.lower():
        if "meditron" in args.model_name_or_path.lower() or "Llama-2" in model_name or "mistral" in args.model_name_or_path.lower():
            
            if args.step_infer:

                print("step_infer 11", model_name)
                i = 0 
                info_query = ''
                for query in query_list:
                    ...
                    print(f"info_query单词数量: {word_count}")

                    if word_count > 200:
                        break

                    print("query", query)
                    result, inference_time = generate_response(query, model, tokenizer, device, 512)
                    # step_result = step_list[i] + result
                    step_result = " " +result
                    word_count = len(step_result.split())
                    step_result = step_result.replace("\n", "")

                    print("------------------------")
                    print(f"{i} step_result单词数量: {word_count}")
                    print("step", i )
                    print(step_result)
                    print("------------------------")

                    info_query_word_count = len(info_query.split())
                    step_result_word_count = len(step_result.split())

                    if info_query_word_count + step_result_word_count < 300:
                        inf_time.append(inference_time)
                        info_query += step_result
                    
                    i += 1

                info_query = info_query.replace("\n", "")
                print("------------------------")  
                ...
                query8= " information: "+ info_query + step_prompt8 +" question: "+ question + "Long form answer(more than 300 words):"

                query8 = query8.replace("\n", "")
                print("query8",query8)

                final_result, inference_time = generate_response(query8, model, tokenizer, device, 1500)
                inf_time.append(inference_time)

                final_result = final_result.replace("\n", "")
                word_count = len(final_result.split())
                print(f"final_result单词数量: {word_count}")
                print(final_result)

                pred = info_query + " Long-Form Answer " +final_result + " END "
                pred = pred.replace("\n", "")
            else:
                print("1",model_name,output_max_length)
                print("query", query)
        ...
        ##llama3-8b-instruct
        elif "Llama-3.2-1B-Instruct" == model_name:

            if args.step_infer:

                print("step_infer", model_name)
                i = 0 
                info_query = ''
                for query in query_list:
                    ...
                    result, inference_time = run_inference_llama3_1B(model, pipe, system, query, model_name)
                ...
                final_result, inference_time = run_inference_llama3_1B(model, pipe, system, query, model_name)
        ...
        ## gpt-3.5-turbo
        ...
        elif model_name_Q == "Qwen/Qwen2.5-1.5B-Instruct":
            
            print("Step Inference:", model_name)
            i = 0
            info_query = ''
            inf_time = []
            ...
            # 遍历查询列表
            for query in query_list:
                # 计算当前 info_query 的单词数量
                current_word_count = len(info_query.split())
                print(f"info_query word count: {current_word_count}")
                ...
                # 调用推理函数，并获取结果和推理时间
                result, inference_time = qwen_generate_response(query, qwen_system, tokenizer, model)
                result = result.replace("\n", "").strip()
                result_word_count = len(result.split())

                print(f"Step {i} Result Word Count: {result_word_count}")
                print(f"Step {i} Result: {result}")

                # 如果 info_query 和当前结果的单词数之和小于300，则合并结果
                combined_word_count = len(info_query.split()) + result_word_count
            ...
            # 去除 info_query 中多余的换行符和空格
            info_query = info_query.replace("\n", "").strip()
            final_word_count = len(info_query.split())
            print(f"Final info_query word count: {final_word_count}")

            # 构建用于生成长篇回答的最终查询
            query8 = f"information: {info_query} {step_prompt8} question: {question} Long form answer (more than 300 words):"
            query8 = query8.replace("\n", "").strip()
            print(f"Query for long-form answer: {query8}")

            # 运行最终推理，获取长篇回答
            final_result, inference_time = qwen_generate_response(query, qwen_system, tokenizer, model)
            inf_time.append(inference_time)

            # 处理最终结果，去除换行符并计算单词数量
            final_result = final_result.replace("\n", "").strip()
            final_result_word_count = len(final_result.split())
            print(f"Final Result Word Count: {final_result_word_count}")
            print(f"Final Result: {final_result}")

            # 组合 info_query 和最终结果
            pred = f"{info_query} Long-Form Answer: {final_result} END"
            pred = pred.replace("\n", "").strip()
        ...
        try:
            print("extract_long_form_answer")
            long_form_answer, all_output = extract_long_form_answer(pred, use_prompt, args.step_infer)
